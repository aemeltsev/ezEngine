...
EZ_CREATE_SIMPLE_TEST(Time, Timestamp)
{
  ...
  EZ_TEST_BLOCK(ezTestBlock::Enabled, "Constructors / Valid Check")
  {
    ...
    EZ_TEST_BOOL_MSG(
      currentTimestamp.GetInt64(ezSIUnitOfTime::Microsecond) < ezTimestamp::CurrentTimestamp().GetInt64(ezSIUnitOfTime::Microsecond), "Sleeping for 10 ms should cause the timestamp to change!");
