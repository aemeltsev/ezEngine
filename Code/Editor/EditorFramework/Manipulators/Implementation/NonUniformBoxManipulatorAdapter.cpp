...
void ezNonUniformBoxManipulatorAdapter::GizmoEventHandler(const ezGizmoEvent& e)
{
  switch (e.m_Type)
  {
    ...
    case ezGizmoEvent::Type::Interaction:
    {
      ...
      if (pAttr->HasSixAxis())
      {
        ChangeProperties(pAttr->GetNegXProperty(), neg.x, pAttr->GetPosXProperty(), pos.x, pAttr->GetNegYProperty(), neg.y, pAttr->GetPosYProperty(), pos.y, pAttr->GetNegZProperty(), neg.z,
