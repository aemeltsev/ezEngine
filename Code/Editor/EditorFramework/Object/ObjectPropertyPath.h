...
{
  bool operator==(const ezPropertyReference& rhs) const { return m_Object == rhs.m_Object && m_pProperty == rhs.m_pProperty && m_Index == rhs.m_Index; }
  ezUuid m_Object;
...
{
public:
  static ezStatus CreatePath(
    const ezObjectPropertyPathContext& context, const ezPropertyReference& prop, ezStringBuilder& out_sObjectSearchSequence, ezStringBuilder& out_sComponentType, ezStringBuilder& out_sPropertyPath);
  ...
  static ezStatus ResolvePath(
    const ezObjectPropertyPathContext& context, ezHybridArray<ezPropertyReference, 1>& out_keys, const char* szObjectSearchSequence, const char* szComponentType, const char* szPropertyPath);
