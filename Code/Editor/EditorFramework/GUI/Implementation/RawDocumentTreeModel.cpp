  ...
  , m_sChildProperty(sChildProperty)
{
  if (!m_sChildProperty.IsEmpty())
  {
    ...
    EZ_ASSERT_DEV(
      pProp != nullptr && (pProp->GetCategory() == ezPropertyCategory::Array || pProp->GetCategory() == ezPropertyCategory::Set), "The visualized object property tree must either be a set or array!");
    EZ_ASSERT_DEV(!pProp->GetFlags().IsSet(ezPropertyFlags::Pointer) || pProp->GetFlags().IsSet(ezPropertyFlags::PointerOwner), "The visualized object must have ownership of the property objects!");
  }
}
  ...
  , m_sNameProperty(szNameProperty)
{
  ...
  EZ_ASSERT_DEV(pProp != nullptr && pProp->GetCategory() == ezPropertyCategory::Member && pProp->GetSpecificType()->GetVariantType() == ezVariantType::String,
