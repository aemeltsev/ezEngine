...
ezStandardJSONWriter::CommaWriter::CommaWriter(ezStandardJSONWriter* pWriter)
{
  ...
  if (m_pWriter->m_StateStack.PeekBack().m_bRequireComma)
  {
    ...
    if (m_pWriter->m_StateStack.PeekBack().m_State == ezStandardJSONWriter::Array || m_pWriter->m_StateStack.PeekBack().m_State == ezStandardJSONWriter::NamedArray)
    {
...
void ezStandardJSONWriter::BeginVariable(const char* szName)
{
  ...
  EZ_IGNORE_UNUSED(state);
  EZ_ASSERT_DEV(state == ezStandardJSONWriter::Empty || state == ezStandardJSONWriter::Object || state == ezStandardJSONWriter::NamedObject, "Variables can only be written inside objects.");
...
void ezStandardJSONWriter::BeginArray(const char* szName)
{
  const ezStandardJSONWriter::State state = m_StateStack.PeekBack().m_State;
  EZ_IGNORE_UNUSED(state);
  EZ_ASSERT_DEV((state == ezStandardJSONWriter::Empty) || ((state == ezStandardJSONWriter::Object || state == ezStandardJSONWriter::NamedObject) && !ezStringUtils::IsNullOrEmpty(szName)) ||
                  ((state == ezStandardJSONWriter::Array || state == ezStandardJSONWriter::NamedArray) && szName == nullptr) || (state == ezStandardJSONWriter::Variable && szName == nullptr),
