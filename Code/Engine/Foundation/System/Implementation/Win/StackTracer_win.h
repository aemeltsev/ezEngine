...
namespace
{
  ...
  typedef BOOL(__stdcall* StackWalk)(DWORD MachineType, HANDLE hProcess, HANDLE hThread, LPSTACKFRAME64 StackFrame, PVOID ContextRecord, PREAD_PROCESS_MEMORY_ROUTINE64 ReadMemoryRoutine,
...
void ezStackTracer::OnPluginEvent(const ezPluginEvent& e)
{
  ...
  if (false) // e.m_EventType == ezPluginEvent::AfterLoading)
  {
    ...
    if (!(*s_pImplementation->getModuleInfo)(currentProcess, moduleAddress, &moduleInfo))
    {
      ...
      FormatMessage(
        FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, err, MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), (LPTSTR)&lpMsgBuf, 0, nullptr);
...
ezUInt32 ezStackTracer::GetStackTrace(ezArrayPtr<void*>& trace, void* pContext)
{
  ...
  if (pContext && s_pImplementation->stackWalk)
  {
    ...
    {
      if (s_pImplementation->stackWalk(
            machine_type, GetCurrentProcess(), GetCurrentThread(), &frame, &context, NULL, s_pImplementation->getFunctionTableAccess, s_pImplementation->getModuleBase, NULL))
      {
