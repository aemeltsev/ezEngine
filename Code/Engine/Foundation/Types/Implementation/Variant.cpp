...
struct ComputeHashFunc
{
  ...
  EZ_FORCE_INLINE void operator()()
  {
    EZ_CHECK_AT_COMPILETIME_MSG(
      sizeof(typename ezVariant::TypeDeduction<T>::StorageType) <= sizeof(float) * 4 && !ezVariant::TypeDeduction<T>::forceSharing, "This type requires special handling! Add a specialization below.");
