...
/// return false.
template <typename T>
struct ezGetTypeClass : public ezTraitInt<(sizeof(*((T*)0) % *((const ezTypeIsMemRelocatable*)0)) == sizeof(ezCompileTimeTrueType)) ? 2 : ezIsPodType<T>::value>
{
};
...
/// \brief mark a class as memory relocatable if the passed type is relocatable or pod.
#  define EZ_DECLARE_MEM_RELOCATABLE_TYPE_CONDITIONAL(T) \
    typename ezConditionToCompileTimeBool<ezGetTypeClass<T>::value == ezTypeIsMemRelocatable::value || ezIsPodType<T>::value>::type operator%(const ezTypeIsMemRelocatable&) const
...
// This macro is only guaranteed to work for classes / structs which don't have any constructor / destructor / assignment operator!
// As arguments you have to list the types of all the members of the class / struct.
#  define EZ_DETECT_TYPE_CLASS(...) ezCompileTimeTrueType operator%(const ezTraitInt<EZ_CALL_MACRO(EZ_CONCAT(EZ_DETECT_TYPE_CLASS_, EZ_VA_NUM_ARGS(__VA_ARGS__)), (__VA_ARGS__))::value>&) const
#endif
