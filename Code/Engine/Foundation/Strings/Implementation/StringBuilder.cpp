...
void ezStringBuilder::Append(const char* pData1, const char* pData2, const char* pData3, const char* pData4, const char* pData5, const char* pData6)
{
  ...
  for (ezUInt32 i = 0; i < uiMaxParams; ++i)
  {
    ...
    EZ_ASSERT_DEBUG(
      pStrings[i] < m_Data.GetData() || pStrings[i] >= m_Data.GetData() + m_Data.GetCapacity(), "Parameter {0} comes from the string builders own storage. This type assignment is not allowed.", i);
...
void ezStringBuilder::AppendWithSeparator(ezStringView optional, ezStringView sText1, ezStringView sText2 /*= ezStringView()*/, ezStringView sText3 /*= ezStringView()*/,
