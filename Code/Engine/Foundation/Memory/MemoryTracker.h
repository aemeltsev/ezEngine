...
struct ezMemoryTrackingFlags
{
  ...
  enum Enum
  {
    None,
    RegisterAllocator =
      EZ_BIT(0),                          ///< Register the allocator with the memory tracker. If EnableAllocationTracking is not set as well it is up to the allocator implementation whether it collects usable stats or not.
...
public:
  ...
  static void AddAllocation(ezAllocatorId allocatorId, ezBitflags<ezMemoryTrackingFlags> flags, const void* ptr, size_t uiSize, size_t uiAlign, ezTime allocationTime);
