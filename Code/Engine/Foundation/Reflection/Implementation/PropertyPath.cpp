...
void ezPropertyPath::WriteToLeafObject(void* pRootObject, const ezRTTI& pType, ezDelegate<void(void* pLeaf, const ezRTTI& pType)> func) const
{
  EZ_ASSERT_DEBUG(m_PathSteps.IsEmpty() || m_PathSteps[m_PathSteps.GetCount() - 1].m_pProperty->GetSpecificType()->GetTypeFlags().IsSet(ezTypeFlags::Class),
...
void ezPropertyPath::ReadFromLeafObject(void* pRootObject, const ezRTTI& pType, ezDelegate<void(void* pLeaf, const ezRTTI& pType)> func) const
{
  EZ_ASSERT_DEBUG(m_PathSteps.IsEmpty() || m_PathSteps[m_PathSteps.GetCount() - 1].m_pProperty->GetSpecificType()->GetTypeFlags().IsSet(ezTypeFlags::Class),
    "To resolve the leaf object the path needs to be empty or end in a class.");
...
void ezPropertyPath::WriteProperty(void* pRootObject, const ezRTTI& pType, ezDelegate<void(void* pLeaf, ezAbstractProperty* pProp, const ezVariant& index)> func) const
{
  EZ_ASSERT_DEBUG(!m_PathSteps.IsEmpty(), "Call InitializeFromPath before WriteToObject");
  ResolvePath(pRootObject, &pType, m_PathSteps.GetArrayPtr().GetSubArray(0, m_PathSteps.GetCount() - 1), true, [this, &func](void* pRootObject, const ezRTTI& pType) {
    auto& lastStep = m_PathSteps[m_PathSteps.GetCount() - 1];
    func(pRootObject, lastStep.m_pProperty, lastStep.m_Index);
  });
}

void ezPropertyPath::ReadProperty(void* pRootObject, const ezRTTI& pType, ezDelegate<void(void* pLeaf, const ezAbstractProperty* pProp, const ezVariant& index)> func) const
{
  EZ_ASSERT_DEBUG(!m_PathSteps.IsEmpty(), "Call InitializeFromPath before WriteToObject");
  ResolvePath(pRootObject, &pType, m_PathSteps.GetArrayPtr().GetSubArray(0, m_PathSteps.GetCount() - 1), false, [this, &func](void* pRootObject, const ezRTTI& pType) {
    auto& lastStep = m_PathSteps[m_PathSteps.GetCount() - 1];
    func(pRootObject, lastStep.m_pProperty, lastStep.m_Index);
  });
}
...
void ezPropertyPath::ResolvePath(
  void* pCurrentObject, const ezRTTI* pType, const ezArrayPtr<const ResolvedStep> path, bool bWriteToObject, const ezDelegate<void(void* pLeaf, const ezRTTI& pType)>& func)
{
  if (path.IsEmpty())
  {
