...
namespace ezInternal
{
  ...
  {
    constexpr EZ_ALWAYS_INLINE ezUInt32 operator()(ezUInt32 hash, const char (&str)[N], size_t i) const { return (hash ^ (str[i + 2] << 16) ^ (str[i + 1] << 8) ^ (str[i + 0])) * MURMUR_M; }
  };

  template <size_t N>
  struct CompileTimeMurmurHash<N, 2>
  {
    constexpr EZ_ALWAYS_INLINE ezUInt32 operator()(ezUInt32 hash, const char (&str)[N], size_t i) const { return (hash ^ (str[i + 1] << 8) ^ (str[i])) * MURMUR_M; }
  };

  template <size_t N>
  struct CompileTimeMurmurHash<N, 1>
  {
    constexpr EZ_ALWAYS_INLINE ezUInt32 operator()(ezUInt32 hash, const char (&str)[N], size_t i) const { return (hash ^ (str[i])) * MURMUR_M; }
  };

  template <size_t N>
  struct CompileTimeMurmurHash<N, 0>
  {
    constexpr EZ_ALWAYS_INLINE ezUInt32 operator()(ezUInt32 hash, const char (&str)[N], size_t i) const { return hash; }
  };

  constexpr ezUInt32 rightShift_and_xorWithPrevSelf(ezUInt32 h, ezUInt32 shift) { return h ^ (h >> shift); }
} // namespace ezInternal
...
constexpr EZ_ALWAYS_INLINE ezUInt32 ezHashingUtils::MurmurHash32String(const char (&str)[N], ezUInt32 uiSeed)
{
  ...
  return ezInternal::rightShift_and_xorWithPrevSelf(
    ezInternal::rightShift_and_xorWithPrevSelf(ezInternal::CompileTimeMurmurHash<N, N - 1>()(uiSeed ^ static_cast<ezUInt32>(N - 1), str, 0), 13) * ezInternal::MURMUR_M, 15);
}
