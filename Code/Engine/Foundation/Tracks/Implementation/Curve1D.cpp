...
void ezCurve1D::CreateLinearApproximation(double fMaxError /*= 0.01f*/, ezUInt8 uiMaxSubDivs /*= 8*/)
{
  ...
  for (ezUInt32 i = 1; i < m_ControlPoints.GetCount(); ++i)
  {
    ...
    ApproximateCurve(m_ControlPoints[i - 1].m_Position, m_ControlPoints[i - 1].m_Position + ezVec2d(m_ControlPoints[i - 1].m_RightTangent.x, m_ControlPoints[i - 1].m_RightTangent.y),
      m_ControlPoints[i].m_Position + ezVec2d(m_ControlPoints[i].m_LeftTangent.x, m_ControlPoints[i].m_LeftTangent.y), m_ControlPoints[i].m_Position, fMaxErrorX, fMaxErrorY, uiMaxSubDivs);
  }
...
void ezCurve1D::ApproximateCurvePiece(const ezVec2d& p0, const ezVec2d& p1, const ezVec2d& p2, const ezVec2d& p3, double tLeft, const ezVec2d& pLeft, double tRight, const ezVec2d& pRight,
  double fMaxErrorX, double fMaxErrorY, ezInt32 iSubDivLeft)
{
