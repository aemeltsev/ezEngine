...
namespace ezIntersectionUtils
{
  ...
  ///   The stride in bytes between each vertex in the pPolygonVertices array. If the array is tightly packed, this will equal sizeof(ezVec3), but it can be larger, if the vertices are interleaved
  ///   with other data.
  /// \return
  ///   True, if the ray intersects the polygon, false otherwise.
  EZ_FOUNDATION_DLL bool RayPolygonIntersection(const ezVec3& vRayStartPos, const ezVec3& vRayDir, const ezVec3* pPolygonVertices, ezUInt32 uiNumVertices, float* out_fIntersectionTime = nullptr,
    ezVec3* out_vIntersectionPoint = nullptr, ezUInt32 uiVertexStride = sizeof(ezVec3)); // [tested]
  ...
  EZ_FOUNDATION_DLL ezVec3 ClosestPoint_PointLineSegment(
    const ezVec3& vStartPoint, const ezVec3& vLineSegmentPos0, const ezVec3& vLineSegmentPos1, float* out_fFractionAlongSegment = nullptr); // [tested]
  ...
  EZ_FOUNDATION_DLL bool Ray2DLine2D(const ezVec2& vRayStartPos, const ezVec2& vRayDir, const ezVec2& vLineSegmentPos0, const ezVec2& vLineSegmentPos1, float* out_fIntersectionTime = nullptr,
    ezVec2* out_vIntersectionPoint = nullptr); // [tested]
