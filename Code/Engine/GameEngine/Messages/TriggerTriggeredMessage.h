...
struct EZ_GAMEENGINE_DLL ezMsgTriggerTriggered : public ezEventMessage
{
  ...
  /// Identifies what the message should trigger. Only stores the hashed string, because one should only check for equality with some expected string. Use ezTempHashedString::GetHash() to assign and
  /// compare the value.
