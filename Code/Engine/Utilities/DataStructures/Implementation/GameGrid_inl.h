...
template <class CellData>
bool ezGameGrid<CellData>::GetRayIntersectionExpandedBBox(
  const ezVec3& vRayStartWorldSpace, const ezVec3& vRayDirNormalizedWorldSpace, float fMaxLength, float& out_fIntersection, const ezVec3& vExpandBBoxByThis) const
{
  const ezVec3 vRayStart = m_RotateToGridspace * (vRayStartWorldSpace - m_vWorldSpaceOrigin);
  const ezVec3 vRayDir = m_RotateToGridspace * vRayDirNormalizedWorldSpace;
