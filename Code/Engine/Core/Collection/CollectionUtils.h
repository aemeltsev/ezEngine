...
namespace ezCollectionUtils
{
  ...
  EZ_CORE_DLL void MergeCollections(ezCollectionResourceDescriptor& result, ezArrayPtr<const ezCollectionResourceDescriptor*> inputCollections);
  ...
  /// string to szAbsFolderpath, which will replace the root, e.g. with an absolute file path. This is just for the file size check within the scope of the function, it will not modify the resource
  /// Id.
  EZ_CORE_DLL void AddResourceHandle(ezCollectionResourceDescriptor& collection, ezTypelessResourceHandle handle, const char* szAssetTypeName, const char* szAbsFolderpath);
