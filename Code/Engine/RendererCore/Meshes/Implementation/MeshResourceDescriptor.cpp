...
void ezMeshResourceDescriptor::Save(ezStreamWriter& stream)
{
  ...
  ezLog::Dev("Compressed mesh data from {0} KB to {1} KB ({2}%%)", ezArgF((float)compressor.GetUncompressedSize() / 1024.0f, 1), ezArgF((float)compressor.GetCompressedSize() / 1024.0f, 1),
...
ezResult ezMeshResourceDescriptor::Load(ezStreamReader& stream)
{
  ...
  if (bCalculateBounds)
  {
    ...
    auto b = m_Bounds;
    ezLog::Info("Calculated Bounds: {0} | {1} | {2} - {3} | {4} | {5}", ezArgF(b.m_vCenter.x, 2), ezArgF(b.m_vCenter.y, 2), ezArgF(b.m_vCenter.z, 2), ezArgF(b.m_vBoxHalfExtends.x, 2),
      ezArgF(b.m_vBoxHalfExtends.y, 2), ezArgF(b.m_vBoxHalfExtends.z, 2));
  }
