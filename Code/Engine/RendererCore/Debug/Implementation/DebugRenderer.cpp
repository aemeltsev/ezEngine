...
namespace
{
  ...
  static PerContextData& GetDataForExtraction(const ezDebugRendererContext& context)
  {
    ...
    const ezUInt32 uiDataIndex = ezRenderWorld::IsRenderingThread() && (doubleBufferedData.m_uiLastRenderedFrame != ezRenderWorld::GetFrameCounter()) ? ezRenderWorld::GetDataIndexForRendering()
  ...
  static void AddTextLines(const ezDebugRendererContext& context, ezStringView text, const ezVec2I32& positionInPixel, float fSizeInPixel,
    ezDebugRenderer::HorizontalAlignment::Enum horizontalAlignment, ezDebugRenderer::VerticalAlignment::Enum verticalAlignment, AddFunc func)
  {
    if (text.IsEmpty())
      return;
...
// static
void ezDebugRenderer::DrawCross(
  const ezDebugRendererContext& context, const ezVec3& globalPosition, float fLineLength, const ezColor& color, const ezTransform& transform /*= ezTransform::IdentityTransform()*/)
{
  if (fLineLength <= 0.0f)
    return;
  ...
  Line lines[3] = {{transform.TransformPosition(globalPosition - xAxis), transform.TransformPosition(globalPosition + xAxis)},
...
void ezDebugRenderer::Draw2DText(const ezDebugRendererContext& context, const ezStringView& text, const ezVec2I32& positionInPixel, const ezColor& color, ezUInt32 uiSizeInPixel /*= 16*/,
  HorizontalAlignment::Enum horizontalAlignment /*= HorizontalAlignment::Left*/, VerticalAlignment::Enum verticalAlignment /*= VerticalAlignment::Top*/)
{
...
void ezDebugRenderer::Draw3DText(const ezDebugRendererContext& context, const ezStringView& text, const ezVec3& globalPosition, const ezColor& color, ezUInt32 uiSizeInPixel /*= 16*/,
  HorizontalAlignment::Enum horizontalAlignment /*= HorizontalAlignment::Left*/, VerticalAlignment::Enum verticalAlignment /*= VerticalAlignment::Top*/)
{
    ...
    if (uiNumTriangleVertices != 0)
    {
      ...
      while (uiNumTriangleVertices > 0)
      {
        ...
        renderViewContext.m_pRenderContext->BindMeshBuffer(
          s_hDataBuffer[BufferType::Triangles3D], ezGALBufferHandle(), &s_VertexDeclarationInfo, ezGALPrimitiveTopology::Triangles, uiNumTriangleVerticesInBatch / 3);
  ...
  // 3D Lines
  {
    ...
    if (uiNumLineVertices != 0)
    {
      ...
      while (uiNumLineVertices > 0)
      {
        ...
        renderViewContext.m_pRenderContext->BindMeshBuffer(
          s_hDataBuffer[BufferType::Lines], ezGALBufferHandle(), &s_VertexDeclarationInfo, ezGALPrimitiveTopology::Lines, uiNumLineVerticesInBatch / 2);
  ...
  // 2D Lines
  {
    ...
    if (uiNumLineVertices != 0)
    {
      ...
      while (uiNumLineVertices > 0)
      {
        ...
        renderViewContext.m_pRenderContext->BindMeshBuffer(
          s_hDataBuffer[BufferType::Lines2D], ezGALBufferHandle(), &s_VertexDeclarationInfo, ezGALPrimitiveTopology::Lines, uiNumLineVerticesInBatch / 2);
  ...
  // 2D Rectangles
  {
    ...
    if (uiNum2DVertices != 0)
    {
      ...
      while (uiNum2DVertices > 0)
      {
        ...
        renderViewContext.m_pRenderContext->BindMeshBuffer(
          s_hDataBuffer[BufferType::Triangles2D], ezGALBufferHandle(), &s_VertexDeclarationInfo, ezGALPrimitiveTopology::Triangles, uiNum2DVerticesInBatch / 3);
    ...
    for (auto itTex = pData->m_texTriangle2DVertices.GetIterator(); itTex.IsValid(); ++itTex)
    {
      ...
      if (uiNum2DVertices != 0)
      {
        ...
        while (uiNum2DVertices > 0)
        {
          ...
          renderViewContext.m_pRenderContext->BindMeshBuffer(
            s_hDataBuffer[BufferType::TexTriangles2D], ezGALBufferHandle(), &s_TexVertexDeclarationInfo, ezGALPrimitiveTopology::Triangles, uiNum2DVerticesInBatch / 3);
    ...
    for (auto itTex = pData->m_texTriangle3DVertices.GetIterator(); itTex.IsValid(); ++itTex)
    {
      ...
      if (uiNumVertices != 0)
      {
        ...
        while (uiNumVertices > 0)
        {
          ...
          renderViewContext.m_pRenderContext->BindMeshBuffer(
            s_hDataBuffer[BufferType::TexTriangles3D], ezGALBufferHandle(), &s_TexVertexDeclarationInfo, ezGALPrimitiveTopology::Triangles, uiNumVerticesInBatch / 3);
