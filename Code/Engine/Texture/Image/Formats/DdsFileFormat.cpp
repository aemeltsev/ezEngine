...
ezResult ezDdsFileFormat::ReadImage(ezStreamReader& stream, ezImage& image, ezLogInterface* pLog, const char* szFileExtension) const
{
  ...
  if ((fileHeader.m_ddspf.m_uiFlags & ezDdpfFlags::ALPHAPIXELS) != 0 || (fileHeader.m_ddspf.m_uiFlags & ezDdpfFlags::RGB) != 0 || (fileHeader.m_ddspf.m_uiFlags & ezDdpfFlags::ALPHA) != 0)
  {
    format = ezImageFormat::FromPixelMask(
      fileHeader.m_ddspf.m_uiRBitMask, fileHeader.m_ddspf.m_uiGBitMask, fileHeader.m_ddspf.m_uiBBitMask, fileHeader.m_ddspf.m_uiABitMask, fileHeader.m_ddspf.m_uiRGBBitCount);

    if (format == ezImageFormat::UNKNOWN)
    {
      ezLog::Error(pLog, "The pixel mask specified was not recognized (R: {0}, G: {1}, B: {2}, A: {3}, Bpp: {4}).", ezArgU(fileHeader.m_ddspf.m_uiRBitMask, 1, false, 16),
        ezArgU(fileHeader.m_ddspf.m_uiGBitMask, 1, false, 16), ezArgU(fileHeader.m_ddspf.m_uiBBitMask, 1, false, 16), ezArgU(fileHeader.m_ddspf.m_uiABitMask, 1, false, 16),
    ...
    {
      ezLog::Error(pLog, "The number of bits per pixel specified in the file ({0}) does not match the expected value of {1} for the format '{2}'.", fileHeader.m_ddspf.m_uiRGBBitCount,
        ezImageFormat::GetBitsPerPixel(format), ezImageFormat::GetName(format));
      return EZ_FAILURE;
    }
  }
  else if ((fileHeader.m_ddspf.m_uiFlags & ezDdpfFlags::FOURCC) != 0)
  {
    ...
    else
    {
      ...
      if (format == ezImageFormat::UNKNOWN)
      {
        ezLog::Error(pLog, "The FourCC code '{0}{1}{2}{3}' was not recognized.", ezArgC((char)(fileHeader.m_ddspf.m_uiFourCC >> 0)), ezArgC((char)(fileHeader.m_ddspf.m_uiFourCC >> 8)),
          ezArgC((char)(fileHeader.m_ddspf.m_uiFourCC >> 16)), ezArgC((char)(fileHeader.m_ddspf.m_uiFourCC >> 24)));
        return EZ_FAILURE;
      }
    }
  }
...
ezResult ezDdsFileFormat::WriteImage(ezStreamWriter& stream, const ezImageView& image, ezLogInterface* pLog, const char* szFileExtension) const
{
  ...
  if (bCubeMap)
  {
    ...
    fileHeader.m_uiCaps2 |= ezDdsCaps2::CUBEMAP | ezDdsCaps2::CUBEMAP_POSITIVEX | ezDdsCaps2::CUBEMAP_NEGATIVEX | ezDdsCaps2::CUBEMAP_POSITIVEY | ezDdsCaps2::CUBEMAP_NEGATIVEY |
  ...
  if (bDxt10)
  {
    ...
    if (uiDxgiFormat == 0)
    {
      ezLog::Error(pLog, "The image needs to be written as a DXT10 file, but no matching DXGI format was found for '{0}'.", ezImageFormat::GetName(format));
      return EZ_FAILURE;
    }
