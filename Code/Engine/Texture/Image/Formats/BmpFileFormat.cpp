...
ezResult ezBmpFileFormat::ReadImage(ezStreamReader& stream, ezImage& image, ezLogInterface* pLog, const char* szFileExtension) const
{
      ...
      switch (uiBpp)
      {
        ...
        case 32:
          ...
          else
          {
            ...
            format = ezImageFormat::FromPixelMask(fileInfoHeaderV4.m_redMask, fileInfoHeaderV4.m_greenMask, fileInfoHeaderV4.m_blueMask, fileInfoHeaderV4.m_alphaMask, uiBpp);
          }
  ...
  else
  {
    ...
    {
      ezLog::Error(pLog, "The number of bits per pixel specified in the file ({0}) does not match the expected value of {1} for the format '{2}'.", uiBpp, ezImageFormat::GetBitsPerPixel(format),
...
{
  return ezStringUtils::IsEqual_NoCase(szExtension, "bmp") || ezStringUtils::IsEqual_NoCase(szExtension, "dib") || ezStringUtils::IsEqual_NoCase(szExtension, "rle");
}
