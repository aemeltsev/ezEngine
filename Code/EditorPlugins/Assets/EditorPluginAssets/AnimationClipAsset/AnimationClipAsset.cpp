...
ezStatus ezAnimationClipAssetDocument::InternalTransformAsset(
...
void ezAnimationClipAssetDocument::ExtractRootMotionFromFeet(ezAnimationClipResourceDescriptor& anim, const ezSkeleton& skeleton) const
{
  ...
  for (ezUInt16 thisFrame = 0; thisFrame < numFrames; ++thisFrame)
  {
    ...
    const ezVec3 smoothedTranslation = (translations[prevFrame2] + translations[prevFrame] + translations[thisFrame] + translations[nextFrame] + translations[nextFrame2]) * 0.2f;
