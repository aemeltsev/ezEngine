  ...
  if (IsTemporary(pObject))
  {
    ...
    else if (type == ezVariantType::Quaternion)
    {
      ...
      for (ezUInt32 c = 0; c < 3; c++)
      {
        ...
        ezUuid track = FindOrAddTrack(pObject, pProp, index, static_cast<ezPropertyAnimTarget::Enum>((int)ezPropertyAnimTarget::RotationX + c), [this, oldValue](const ezUuid& trackGuid) {
...
ezStatus ezPropertyAnimObjectAccessor::SetCurveCp(
  const ezDocumentObject* pObject, const ezAbstractProperty* pProp, ezVariant index, ezPropertyAnimTarget::Enum target, double fOldValue, double fNewValue)
{
