...
namespace
{
  ...
  {
    RecursiveMeshImport(
      ezObjectAccessorBase& sceneAccessor, const ezDocumentObjectManager& objectManager, const ezModelImporter::Scene& scene, const ezHashTable<const ezModelImporter::Mesh*, ezString>& rawMeshToFile)
...
void ezQtSceneImportDlg::on_accepted()
{
  ...
  // Load model.
  ezSharedPtr<ezModelImporter::Scene> rawScene = ezModelImporter::Importer::GetSingleton()->ImportScene(inputFilename, ezModelImporter::ImportFlags::All, true);
  if (!rawScene)
  {
  ...
  for (auto meshIt = rawScene->GetMeshes().GetIterator(); meshIt.IsValid(); ++meshIt)
  {
    ...
    // Create document.
    ezMeshAssetDocument* meshDocument = ezDynamicCast<ezMeshAssetDocument*>(ezQtEditorApp::GetSingleton()->CreateDocument(meshFilename, ezDocumentFlags::None));
    if (!meshDocument)
    {
...
void ezQtSceneImportDlg::validatePaths()
{
  ...
  ezString inputFilename = lineEditSourceFilename->text().toUtf8().data();
  ezString outputFilename = lineEditOutputFilename->text().toUtf8().data();
  if (inputFilename.IsEmpty() || outputFilename.IsEmpty() || ezPathUtils::GetFileExtension(outputFilename).IsEqual_NoCase("ezScene") == false)
  {
...
void ezQtSceneImportDlg::on_InputBrowse_clicked()
{
  ...
  QString filename = QFileDialog::getOpenFileName(
    QApplication::activeWindow(), QLatin1String("Input Model"), ezToolsProject::GetSingleton()->GetProjectDirectory().GetData(), filter, nullptr, QFileDialog::Option::DontResolveSymlinks);
...
{
  QString filename = QFileDialog::getSaveFileName(QApplication::activeWindow(), QLatin1String("Output Scene"), ezToolsProject::GetSingleton()->GetProjectDirectory().GetData(), "ezScene (*.ezScene)",
    nullptr, QFileDialog::Option::DontResolveSymlinks | QFileDialog::Option::DontConfirmOverwrite);
