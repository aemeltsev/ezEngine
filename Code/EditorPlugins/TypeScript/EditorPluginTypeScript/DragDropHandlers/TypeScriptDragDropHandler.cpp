...
void ezTypeScriptComponentDragDropHandler::OnDragBegin(const ezDragDropInfo* pInfo)
{
  ...
  else
  {
    ...
    {
      CreateDropObject(pInfo->m_vDropPosition, "ezTypeScriptComponent", "Script", GetAssetGuidString(pInfo), pInfo->m_TargetObject, pInfo->m_iTargetObjectInsertChildIndex);
    }
  }
