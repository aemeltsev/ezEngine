      ...
      else
      {
        ...
        for (int i = 0; i < diagram.numsites; ++i)
        {
          ...
          while (e)
          {
            ...
            // Front side
            ezUInt32 frontIdx[3];
            frontIdx[0] = g.AddVertex(
              ezVec3(site->p.x, fHalfThickness, site->p.y), ezVec3(0, 1, 0), ezVec2((site->p.x + halfSize.x) * fInvWidth, (site->p.y + halfSize.z) * fInvHeight), ezColor::White, iPieceMatrixIndex);
            frontIdx[1] = g.AddVertex(ezVec3(e->pos[0].x, fHalfThickness, e->pos[0].y), ezVec3(0, 1, 0), ezVec2((e->pos[0].x + halfSize.x) * fInvWidth, (e->pos[0].y + halfSize.z) * fInvHeight),
              ezColor::White, iPieceMatrixIndex);
            frontIdx[2] = g.AddVertex(ezVec3(e->pos[1].x, fHalfThickness, e->pos[1].y), ezVec3(0, 1, 0), ezVec2((e->pos[1].x + halfSize.x) * fInvWidth, (e->pos[1].y + halfSize.z) * fInvHeight),
              ezColor::White, iPieceMatrixIndex);
            ...
            backIdx[0] = g.AddVertex(ezVec3(site->p.x, -fHalfThickness, site->p.y), ezVec3(0, -1, 0),
              ezVec2(1.0f - ((site->p.x + halfSize.x) * fInvWidth), 1.0f - ((site->p.y + halfSize.z) * fInvHeight)), ezColor::White, iPieceMatrixIndex);
            backIdx[1] = g.AddVertex(ezVec3(e->pos[0].x, -fHalfThickness, e->pos[0].y), ezVec3(0, -1, 0),
              ezVec2(1.0f - ((e->pos[0].x + halfSize.x) * fInvWidth), 1.0f - ((e->pos[0].y + halfSize.z) * fInvHeight)), ezColor::White, iPieceMatrixIndex);
            backIdx[2] = g.AddVertex(ezVec3(e->pos[1].x, -fHalfThickness, e->pos[1].y), ezVec3(0, -1, 0),
              ezVec2(1.0f - ((e->pos[1].x + halfSize.x) * fInvWidth), 1.0f - ((e->pos[1].y + halfSize.z) * fInvHeight)), ezColor::White, iPieceMatrixIndex);
  ...
  for (ezUInt32 i = 0; i < m_PieceActors.GetCount(); ++i)
  {
    ...
    m_PieceActors[i] = pActor;
    {
      ...
      // Add initial impulse
      {
        ...
        PxRigidBodyExt::addForceAtPos(*pActor, ezPxConversionUtils::ToVec3(modifiedImpulse), ezPxConversionUtils::ToVec3(vPointOfBreakage), PxForceMode::eIMPULSE);
      }
    }
  }
}
